<?php
	...
	require_once("servidor.php");
		...
		try {
			...
			$query_in_padron= "DELETE FROM TIPOS_LICENCIAS_AUTORIZACIONES WHERE ID=?";
			$pst_in_alum=$conn->prepare($query_in_padron);
			$pst_in_alum->execute($par_padron);

			$conn->commit();
			$messages[] ="Tipo de Licencia o Autorizacion eliminado correctamente.";
...
if (isset($errors)){
			...
			if (isset($messages)){
				...
				<div class="alert alert-success" role="alert">
						<button type="button" class="close" data-dismiss="alert" style="margin-bottom: 0px; margin-top: 6px;">&times;</button>
