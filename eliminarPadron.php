<?php
	...
	require_once("servidor.php");
		...
		try {
			...
			$query_in_padron= "DELETE FROM CONTRIBUYENTE WHERE ID=?";
			$pst_in_alum=$conn->prepare($query_in_padron);
			$pst_in_alum->execute($par_padron);

			$conn->commit();
			$messages[] ="El padron se ha eliminado correctamente.";
			echo("<script>window.location='gestionarPadronContribuyente.php'</script>");
		} catch (Exception $e) {
